            ...
            <React.Fragment key={post.slug}>
              <div className="border border-slate-150 bg-slate-50/50 rounded-xl p-6 shadow-sm hover:shadow-md hover:border-indigo-500/30 transition duration-300">
                ...
                <h2 className="text-xl font-bold mt-2 mb-3 text-slate-950">
                  <Link href={`/blog/${post.category}/${post.slug}`} className="hover:text-indigo-600 transition duration-200">
                    {post.title}
                  </Link>
                </h2>
                ...
                <Link href={`/blog/${post.category}/${post.slug}`} className="text-sm text-indigo-600 font-medium hover:underline inline-flex items-center gap-1">
                  Đọc tiếp &rarr;
                </Link>
              </div>
              {idx === 1 && (
