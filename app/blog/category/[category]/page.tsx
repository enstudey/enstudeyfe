...
export default async function BlogCategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  return (
    <main className="max-w-4xl mx-auto py-12 px-4 bg-white text-slate-900 min-h-screen">
      ...
      <div className="mb-8">
        <Link href="/blog" className="text-sky-600 hover:underline inline-flex items-center gap-1 font-medium">
          ...
          <React.Fragment key={post.slug}>
            <div className="border border-slate-150 bg-slate-50/50 rounded-xl p-6 shadow-sm hover:shadow-md hover:border-sky-500/30 transition duration-300">
              <h2 className="text-xl font-bold mb-3 text-slate-955">
                <Link href={`/blog/${post.slug}`} className="hover:text-sky-600 transition duration-200">
                  {post.title}
                </Link>
              </h2>
              ...
              <Link href={`/blog/${post.slug}`} className="text-sm text-sky-600 font-medium hover:underline inline-flex items-center gap-1">
                Đọc tiếp nha &rarr;
              </Link>
            </div>
            {idx === 1 && (
