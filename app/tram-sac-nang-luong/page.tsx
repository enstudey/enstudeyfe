        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {Array.from({ length: Math.ceil((donors.length * 2) / 2) }).map((_, rowIndex) => {
                  ...
                  return (
                    <div key={rowIndex} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {first && (
                        <div className="bg-slate-50/50 border border-slate-200/50 px-4 py-3.5 rounded-2xl flex items-center justify-between hover:border-sky-500/20 transition duration-200">
                          <span className="font-bold text-slate-800 text-xs truncate">
                            {first.name}
                          </span>
                          <span className="text-[10px] font-extrabold px-2 py-0.5 bg-sky-100 text-sky-600 rounded-md shrink-0">
                            {first.amount}
                          </span>
                        </div>
                      )}
          ...
          <div className="lg:col-span-4 bg-card border border-border rounded-3xl p-6 md:p-8 shadow-xs space-y-6 flex flex-col items-center">
            ...
            <a
              ...
              className="mx-auto w-44 h-44 border-4 border-sky-500/10 rounded-2xl overflow-hidden shadow-md bg-slate-50 flex items-center justify-center cursor-zoom-in block"
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-left text-xs space-y-2 w-full">
              ...
              <div className="flex justify-between items-center gap-2 pt-1 border-t border-slate-100">
                ...
                <span className="font-bold text-sky-600">{account_no}</span>
              </div>
            </div>
          </div>
