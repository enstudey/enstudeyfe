    ...
    <div className="relative w-full overflow-hidden">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
        ...
        {pointsCoords.length > 1 ? (
          <polyline
            points={pointsString}
            className="fill-none stroke-blue-600 dark:stroke-blue-400 stroke-[3] stroke-linecap-round stroke-linejoin-round"
          />
        ) : (
          <circle
            cx={pointsCoords[0].x}
      ...
      {hoveredPoint && (
        <div className="absolute top-2 right-2 bg-slate-900/90 text-white text-xs px-3 py-2 rounded-xl shadow-lg backdrop-blur-sm border border-slate-700 animate-in fade-in">
          <div className="font-bold text-blue-300">{hoveredPoint.title}</div>
