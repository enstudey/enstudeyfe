...
export default function DonateCardDetailed() {
  ...
  return (
    <>
      <div
        className="w-full bg-card border border-border rounded-3xl p-6 relative shadow-sm transition duration-300 hover:shadow-md"
        data-testid="donate-card-detailed"
      >
        {/* Nút đóng X ở góc trên bên phải */}
        <button
          onClick={handleCloseAction}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-650 w-6 h-6 rounded-full hover:bg-slate-100 flex items-center justify-center text-sm transition cursor-pointer"
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <h4 className="text-sm font-bold text-foreground flex items-center gap-1.5 pr-6">
          ...
          <Button
            ...
            className="w-full py-5 text-xs font-bold rounded-xl shadow-sm cursor-pointer"
          >
