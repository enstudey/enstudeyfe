...
export default function DonateSection() {
  ...
  return (
    ...
    >
      <div className="bg-sky-50/30 border border-sky-500/10 rounded-3xl p-6 flex flex-col md:flex-row gap-6 items-center">
        {/* Thư ngỏ */}
        ...
        {/* Mã QR */}
        <div className="w-44 h-44 border-4 border-dashed border-sky-500/20 rounded-2xl overflow-hidden shadow-md shrink-0 bg-slate-50 flex items-center justify-center">
          <Image
          ...
          <div className="relative w-full">
            ...
            >
              {config.donors.map((donor, idx) => (
                <div
                  key={idx}
                  className="bg-slate-50 border border-slate-200/50 px-3.5 py-3 rounded-2xl shrink-0 w-48 snap-start shadow-2xs hover:border-sky-500/20 transition-colors flex items-center justify-between"
                >
                  <span className="font-bold text-slate-800 truncate pr-1 text-xs">
                    {donor.name}
                  </span>
                  <span className="text-[10px] font-extrabold px-1.5 py-0.5 bg-sky-100 text-sky-600 rounded-md shrink-0">
                    {donor.amount}
                  </span>
                </div>
              ))}
